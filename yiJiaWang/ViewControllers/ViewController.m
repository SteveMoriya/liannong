...
static NSString *homePageURL = @"http://www.lnyuns.com/app/initaction!loadappleftmenu.action?visitType=1";  //首页URL
...
//    }
    ...
    if ([dic[@"function"] isEqualToString:@"WXPay"]) {
...
- (void)paySuccess {
    ...
    NSString *urlString;
    urlString = [NSString stringWithFormat:@"%@",@"http://www.lnyuns.com/app/topayAction!alipaySuccessAPP.action?resultCode=1"];
    [_webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
- (void)payFailed {
    ...
    NSString *urlString;
    urlString = [NSString stringWithFormat:@"%@", @"http://www.lnyuns.com/app/topayAction!alipaySuccessAPP.action?resultCode=0"];
    [_webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
